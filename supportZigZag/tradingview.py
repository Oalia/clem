...
def generate_csv(a):
    ...
    # filepath = path.relpath('..\\realtime\\{ticker}.csv', basepath)
    with open("filepath.csv", mode='a', newline='') as data_file:
...
symbol_map = {"symbol": symbol, "adjustment": "splits"}

# "={\"symbol\": \"BINANCE:BTCBUSD\" ,\"adjustment\":\"splits\"}"
sendMessage(ws, "resolve_symbol", [chart_session, "symbol_1", "=" + json.dumps(symbol_map) ])
sendMessage(ws, "create_series", [chart_session,"s1","s1","symbol_1","1",300])
sendMessage(ws, "quote_fast_symbols", [session, json.dumps(symbol)])
